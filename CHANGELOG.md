## 0.2.41-dev - Add Tok'ra decoded mission lead

- After the Tok'ra encoded intelligence packet is analyzed, schedule a delayed Tok'ra follow-up contact.
- Record a persistent decoded mission-lead state and expose it through the Tok'ra channel status report.
- Frame the decoded lead as an isolated Goa'uld relay clue for a future mission without generating a world site yet.
- Add a debug action to force the decoded lead for testing.
- Keep the step non-material: no site, raid, healing, reinforcements, trade, recruitment or reward is created.
- Align mod metadata and C# assembly version with `0.2.41`.
