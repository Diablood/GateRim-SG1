...
- Restore the missing published changelog sections for `0.3.94-dev` and
  `0.3.96-dev` without rewriting their tags.
- Restore durable test coverage for the final basin, Goa'uld hand-device,
  Tok'ra mission-object and Prim'ta larval-item visual milestones.
- Reconcile the visual register with its authoritative inventory table and
  remove stale branch, milestone, duplicated-row and obsolete-finding content.

### Added

- Add `tools/check-documentation-consistency.ps1` and its Windows wrapper.
- Add negative regression fixtures covering missing or duplicated history,
  durable-test deletion, stale visual summaries, roadmap misuse, procedure drift
  and unfinished publication wording.
- Protect published changelog continuity from `v0.3.67-dev` onward and durable
  testing continuity from `v0.3.93-dev` onward.
- Protect the active-only role of `docs/ROADMAP.md`.
- Integrate the documentation audit into the aggregate project-consistency
  command and add `-RequirePublicationReady`.

### Changed

- Update the publication procedure so temporary PowerShell scripts never belong
  at repository root and the final documentation-ready gate runs before staging.
- Return the roadmap to decided future work, with the Jaffa helmet-mode command
  icon recorded as the next visual target.
- Advance public metadata to `0.3.100-dev` and assembly metadata to `0.3.100.0`.
...
- Replace temporary or shared Tok'ra mission-object art with dedicated validated
  visuals for the cipher module, encoded intelligence packet, observation device,
  organic dead drop, Goa'uld relay control node and secure communicator.
- Reuse the final observation-device family intentionally for the deployed
  observation point and remove its obsolete dedicated texture family.
- Finalize the priority delivery marker with a thicker RimWorld-style black
  outline and integrated light transparency.
- Preserve all world-event site textures under
  `Textures/World/WorldObjects/Expanding/Sites` as maintainer-validated final art.
- Preserve mission mechanics, trust gating, research, balance and serialized
  identifiers.
- Record the relay-site overhead-mountain collapse issue as a deferred defect.
- Add protected wiki media references and advance metadata to `0.3.95-dev`.
- Requires the standard forced build and publication checks.
