...
> Dernière révision : `0.3.105-dev`
>

Cette page distingue les fonctionnalités déjà disponibles du contenu encore en préparation.

La version `0.3.105-dev` finalise le rendu mobile de la reine Goa'uld.
Trois PNG directionnels transparents, un contour épais et `drawSize = 0.95`
remplacent l'ancien symbiote adulte agrandi. L'incident rare, les statistiques,
la capture et la production d'un symbiote immature après trois jours de
récupération restent inchangés.
...
identités ni les règles d'implantation. La reine utilise désormais sa propre
...
convois Jaffa libres et une texture finale transparente. Les PawnKinds Jaffa
compatibles peuvent le sélectionner dans les bases, raids, caravanes et missions,
tandis que le briseur de murs Tok'ra reste limité au Ma'Tok.
...
|---|---|---|
| Reine Goa'uld directionnelle | Famille `Graphic_Multi` dédiée, trois PNG `128×128`, ouest miroir, `drawSize = 0.95`, proportions nord/sud ajustées, incident et production préservés | 0.3.105-dev |
...
| Longévité jaffa liée au Prim'ta | Fournie par l'état de santé du Prim'ta ; ancien gène prototype supprimé | 0.1.3-dev / nettoyage 0.3.89-dev |
...
| Groupes Tok'ra | Profils internes `Combat` et `Peaceful` réutilisés par la présence mondiale masquée et les incidents Tok'ra ; cache médical, signal, pistes de planque, cache de suivi, marqueur mondial, planque visitable, contact pacifique et interaction légère de contact | 0.1.41-dev / présence 0.2.7-dev / cache 0.2.12-dev / signal 0.2.13-dev / pistes 0.2.14-dev / suivi 0.2.15-dev / marqueur 0.2.16-dev / site 0.2.17-dev / contact 0.2.18-dev / dialogue 0.2.20-dev / briefing 0.2.21-dev / icône de planque 0.3.51-dev |
...
| Domaines des Grands Maîtres Goa'uld | Faction mondiale hostile visible : trois factions proposées par défaut mais quantité réductible dans la liste vanilla, colonies limitées, résumé provisoire `Jaffa : 100 %`, icône dédiée, Grand Maître réel comme dirigeant et caste d'hôtes minoritaire ; deux domaines territoriaux actifs suffisent au cadre stratégique | 0.1.61-dev / présence 0.2.1-dev / hôtes 0.2.3-dev / icône 0.3.50-dev / défaut et seuil territorial 0.3.83-dev |
...
| Relations entre domaines Goa'uld | Chaque paire conserve neutralité, rivalité, conflit ouvert, trêve ou alliance, synchronisés vers les relations vanilla `Neutral/Hostile/Ally` sans toucher au joueur ; sous Commandement SG-1, les influences de doctrine et de pression restent bornées, et les réactions alliées peuvent aller jusqu'à une rupture après échec majeur | 0.3.66-dev / conflit 0.3.68-dev / alliance 0.3.73-dev / renforts 0.3.78-dev / conjoints 0.3.79-dev / doctrines 0.3.80-dev / représailles 0.3.81-dev / rupture 0.3.82-dev / cohérence vanilla 0.3.83-dev |
...
| Mission jouable de sabotage du relais Tok'ra | Carte temporaire fortifiée, garnison Jaffa, sabotage progressif, renforts différés, évacuation et débriefing avec conséquence sur la confiance | 0.2.45-dev à 0.2.47-dev / icône dédiée 0.3.51-dev |
