...
or retained as a factionless prisoner when generated by the domain
    +
...
For a previously possessed player pawn, `displacedHostFaction` restores player
membership as before. A generated Goa'uld caste host has no earlier human
faction to restore: after successful extraction, a captured such pawn leaves
the System Lord domain, remains a factionless colony prisoner and can be
recruited or released through the normal Prisoner tab. Unrelated pawn origins
are not rewritten.
...
- make a captured generated caste host factionless, still imprisoned and
  recruitable after extraction;
