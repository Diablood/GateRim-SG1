\
    ...
    ## Current milestone
    ### 0.1.63-dev — Add automatic Prim'ta provisioning for Goa'uld Jaffa

    The first generated Goa'uld-aligned Jaffa servants now receive one initial
    Prim'ta automatically. The initializer targets only the two hidden System
    Lord faction pawn kinds and records each pawn once.

    Removing the Prim'ta later does not create an artificial replacement. This
    preserves a clean foundation for future extraction, dependency and tretonin
    gameplay.

    The faction remains hidden and cannot generate world settlements, natural
    raids or traders yet. Equipment and visual markings remain intentionally
    deferred.
